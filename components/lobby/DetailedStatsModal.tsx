    ...
    >
      <div className="space-y-6">
          ...
          {strategyStats.length > 0 && (
            <div>
              <h3 className="mb-3 text-lg font-semibold text-highlight">
                전략바둑
              </h3>
              <div className="space-y-2">
                {strategyStats.map((stat) => {
                  ...
                  return (
                    <div
                      key={stat.gameType}
                      className="flex items-center justify-between rounded-lg border border-color bg-tertiary/30 p-4 hover:bg-tertiary/50 transition-all"
                    >
                      <div className="flex-1">
                        <p className="font-bold text-on-panel">{gameType?.name || stat.gameType}</p>
                      </div>
                      <div className="flex items-center gap-3 text-sm">
                        <span className="text-green-400 font-semibold">
                          승 {stat.wins}
                        </span>
                        <span className="text-red-400 font-semibold">패 {stat.losses}</span>
                        <span className="text-highlight font-bold">
                          승률 {stat.total > 0 ? Math.round((stat.wins / stat.total) * 100) : 0}%
                        </span>
                        <span className="text-secondary">총 {stat.total}경기</span>
                      </div>
                    </div>
                  );
                })}
